...
- **branch**（分支）, 模式从**RUN_REFCASE**和**RUN_REFDATE**设定的 restart file（文件名中有` *.r.* ` ）开始继续往下模拟, 同restart run（当CONTINUE_RUN是TRUE时），但需要有相应时间的restart文件。当两个case之间的各种参数都一样时，他们跑出来的数据也是一模一样的。常用于敏感性实验。    
...
- **hybrid**，同startup一样初始话，只是其初始化数据来自以往跑的一个算例的restart数据，有点类似branch但该设置的约束较少。也需要设定**RUN_REFCASE**和**RUN_REFDATE** ，当其各种参数设置同reference case时，其气候态数据也同reference case，但每一天的数据不一定一样。   
在这个选项中，开始模拟的日期可以用 `$RUN_STARTDATE` 设置。在hybrid的初始化中，海洋模式也是在模式模拟的第二天开始耦合。在没有restart file时是冷启动。
...
User Guide 的解释
http://www.cesm.ucar.edu/models/cesm1.2/cesm/doc/usersguide/c1113.html
